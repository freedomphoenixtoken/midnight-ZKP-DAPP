...
import { Gift, Sparkles, CheckCircle2, Lock, Wallet, Loader2 } from 'lucide-react';
...
export function AirdropWidget({ 
  ...
  const generateProof = async () => {
    ...
    try {
      ...
      onProofGenerated?.(proofHash, { ...eligibilityData, verificationCode: zkProof?.verificationCode });
    } catch (err) {
      console.error('API error, using fallback:', err);
      const timestamp = Date.now();
      const randomString = Math.random().toString(36).substring(7);
      const hash = `zk_airdrop_${address.substring(0, 8)}_${timestamp}_${randomString}`;
      ...
      setProofHash(hash);
      onProofGenerated?.(hash, eligibilityData);
    } finally {
  ...
  const sendVerificationTransaction = async () => {
    ...
    try {
      ...
      alert(`✓ Transaction signed and submitted successfully!\n\nVerification Code: ${verificationCode}\n\nTransaction ID: ${mockUuid}\n\nThe marketplace can now verify your airdrop eligibility from the XRPL blockchain.`);
    } catch (error) {
    ...
    <div className={`p-6 rounded-2xl border ${theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} shadow-lg`}>
      ...
      {proofHash && (
        <div className={`mt-4 p-3 rounded-lg ${theme === 'dark' ? 'bg-gray-700' : 'bg-green-50'}`}>
          ...
          {verificationCode && (
            <div className={`p-2 rounded ${theme === 'dark' ? 'bg-gray-600' : 'bg-orange-50'} mb-3`}>
              <div className="text-xs text-gray-500 mb-1">Verification Code</div>
              <div className="text-xs font-mono text-orange-900 dark:text-orange-300 break-all">{verificationCode}</div>
            </div>
          )}
          {verificationCode && (
            <button

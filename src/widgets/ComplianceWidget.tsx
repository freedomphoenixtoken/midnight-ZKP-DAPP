...
import { Shield, Sparkles, CheckCircle2, Lock, Wallet, Loader2 } from 'lucide-react';
...
export function ComplianceWidget({ 
  ...
  const generateProof = async () => {
    ...
    try {
      ...
      const hash = `zk_compliance_${address.substring(0, 8)}_${timestamp}_${randomString}`;
    ...
    <div className={`p-6 rounded-2xl border ${theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} shadow-lg`}>
      ...
      {proofHash && (
        <div className={`mt-4 p-3 rounded-lg ${theme === 'dark' ? 'bg-gray-700' : 'bg-green-50'}`}>
          ...
          {verificationCode && (
            <div className={`p-2 rounded ${theme === 'dark' ? 'bg-gray-600' : 'bg-blue-50'} mb-3`}>
              <div className="text-xs text-gray-500 mb-1">Verification Code</div>
              <div className="text-xs font-mono text-blue-900 dark:text-blue-300 break-all">{verificationCode}</div>
            </div>
          )}
          {verificationCode && (
            <button
